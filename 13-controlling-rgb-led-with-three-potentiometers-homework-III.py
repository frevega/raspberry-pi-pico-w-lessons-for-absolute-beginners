...
timer = Timer(mode = Timer.PERIODIC, period = 100, callback = lambda t:print(f"(pin: {Pin(23).value()})\tr: {leds[0].duty_u16()}\tg: {leds[1].duty_u16()}\tb: {leds[2].duty_u16()}"))
...
#             leds[i].duty_u16(65535 -1 * (65535 if pots[i].read_u16() >= 65535 else pots[i].read_u16()))         

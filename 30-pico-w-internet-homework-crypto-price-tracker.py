...
class NetworkStation(network.WLAN):
    ...
    def __prepareSocket(self) -> (str, int):
...
class CMCApiClient:
    def __init__(self, debug = False):
        ...
        self.invertColorsScreenTimer = machine.Timer(-1)
        ...
        # Program timers
        self.timers = [machine.Timer(-1), machine.Timer(-1)]
        
        # Debugging flag
        self.debug = debug
    
    def prepreCMCURL(self):
        symbols = ",".join([b for b, a in self.checked_values if a != ""])
    ...
    def displayCMCData(self, pull_from_server = True, invert_colors = False):
        ...
        if self.server_response["status"]["error_code"] == 0:
            ...
            self.screen.fill(0)
            items = [b for b, a in self.checked_values if a != ""]
            for symbol in items:
            ...
            self.screen.writeText("PRICE", x = price_title_x_pos)
        ...
        if invert_colors:
            self.invertColorsScreenTimer.init(mode = machine.Timer.PERIODIC, period = 50, callback = lambda t:self.invertScreenColors())
    ...
    def webPage(self) -> str:
        ...
        <html>
            ...
            <body>
                ...
                <form>
                    <input type="checkbox" id="btc" name="btc" {self.checked_values[0][1]}/>
                    <label for="btc">BTC</label><br/>
                    <input type="checkbox" id="eth" name="eth" {self.checked_values[1][1]}/>
                    <label for="eth">ETH</label><br/>
                    <input type="checkbox" id="usdt" name="usdt" {self.checked_values[2][1]}/>
                    <label for="usdt">USDT</label><br/> 
                    <input type="checkbox" id="bnb" name="bnb" {self.checked_values[3][1]}/>
        ...
        while True:
            ...
            try:
                ...
                if len(request) > 0:
                    ...
                    self.timers[1].init(mode = machine.Timer.PERIODIC, period = 15000, callback = lambda t:program.displayCMCData(invert_colors = True))
                    self.displayCMCData()
            except IndexError:
                print("IndexError")
                pass
...
if __name__ == "__main__":
    flipScreenTimer = machine.Timer(-1)
    try:
        program = CMCApiClient(debug = False)
        flipScreenTimer.init(mode = machine.Timer.PERIODIC, period = 50, callback = lambda t:program.readFlipButton())
        ...
        if  program.networkStation.isconnected():   
            ...
            program.timers[0].init(mode = machine.Timer.PERIODIC, period = 50, callback = lambda t:program.updateChangeColumnButton())
            program.timers[1].init(mode = machine.Timer.PERIODIC, period = 15000, callback = lambda t:program.displayCMCData(invert_colors = True))
            program.start()
